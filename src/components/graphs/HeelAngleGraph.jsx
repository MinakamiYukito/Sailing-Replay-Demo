...
const HeelAngleGraph = ({
  ...
  useEffect(() => {
    const newLineColors = heelAngle.map((_, index) => colors[index % colors.length]);
  ...
  useEffect(() => {
    ...
    const options = {
      ...
      series: [
        { label: "Heel Angle", stroke: "black" },
        ...(window.isLiveData ? liveData.map((data, index) => ({
          label: `Live Boat-${index + 1}`,
          stroke: lineColors[index],
          value: () => {
            // Handle the live data value here
            return data.heelAngle ? data.heelAngle.slice(-1)[0].toFixed(0) : 0; // Fix to no decimal places
          },
        })): heelAngle.map((_, index) => ({
          label: `Boat-${index + 1}`,
          stroke: lineColors[index],
          value: (_, rawValue) =>
            //fix to no decimal places
            frameIndex !== -1 ? heelAngle[index][frameIndex].toFixed(0) : 0, 
        }))),
      ],
    };    
    ...
    const data = [
      ...
      ...(window.isLiveData 
        ? liveData.map((dataEntry) => dataEntry.heelAngle) // Y-axis values from live data
        : heelAngle.map((angleData) => angleData.slice(0, frameIndex + 1)) // Y-axis values from regular data
      ),
    ];
  ...
  }, [heelAngle, globalClockTime, lineColors, filteredTime, frameIndex, liveData]);
