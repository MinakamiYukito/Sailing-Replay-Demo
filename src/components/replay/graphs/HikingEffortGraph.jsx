...
const HikingEffortGraph = ({
  ...
  // Filter time to include every 8th element
  const filteredTime = time.filter((_, index) => (index + 1) % 8 === 0);

  useEffect(() => {
    const newLineColors = hiking.map((_, index) => colors[index % colors.length]);
  ...
  useEffect(() => {
    ...
    const options = {
      ...
      series: [
        { label: "Hiking Effort", stroke: "black" },
        ...(window.isLiveData ? liveData.map((data, index) => ({
          label: `Live Boat-${index + 1}`,
          stroke: lineColors[index],
          value: () => {
            // Handle the live data value here
            return data.hikingEffect ? data.hikingEffect.slice(-1)[0].toFixed(1) : 0; // Fix to no decimal places
          },
        })): hiking.map((_, index) => ({
          label: `Boat-${index + 1}`,
          stroke: lineColors[index],
          // Fix the value to 1 decimal place
          value: (_, rawValue) => (frameIndex !== -1 ? hiking[index][frameIndex].toFixed(1) : 0),
        }))),
      ],
    };    
    ...
    const data = [
      ...
      ...(window.isLiveData 
        ? liveData.map((dataEntry) => dataEntry.hikingEffect) // Y-axis values from live data
        : hiking.map((hikingData) => hikingData.slice(0, frameIndex + 1)) // Y-axis values from regular data
      ),
    ];
  ...
  }, [hiking, globalClockTime, lineColors, filteredTime, frameIndex, liveData]);
